...
>
> > 이번 작업에서 백앤드에서는 노드js를 사용해보려고 했다. API연결을 백으로 해야한다면, 지금 사용하면 되겠다 싶어 알아보았는데 JavaScript의 또다른 라이브러리로 '리액트를 기반으로하는 라이브러리'인 Node.js. React와는 또다른 라이브러리로 구조를 다시 작업해야 한다.
> >
> > > Node.js를 공부해보자!
